...
export class UsuarioService {
    ...
    async create(usuario: Usuario): Promise<Usuario> {
        ...
        const calcularIMC = (peso: number, altura: number): { imc: number; classificacao: string } => {
            if (peso <= 0 || altura <= 0) {
                throw new HttpException('Peso e altura devem conter valores válidos', HttpStatus.BAD_REQUEST);
            }
